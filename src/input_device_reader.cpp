...
void InputEventProcessor::setButton(std::uint16_t code, bool pressed) {
  switch (code) {
    case BTN_JOYSTICK + 0: state_.cross_pressed = pressed; break;
    case BTN_JOYSTICK + 1: state_.square_pressed = pressed; break;
    case BTN_JOYSTICK + 2: state_.circle_pressed = pressed; break;
    case BTN_JOYSTICK + 3: state_.triangle_pressed = pressed; break;
    case BTN_JOYSTICK + 4: state_.r1_pressed = pressed; break;
    case BTN_JOYSTICK + 5: state_.l1_pressed = pressed; break;
    case BTN_JOYSTICK + 6: state_.r2_pressed = pressed; break;
    case BTN_JOYSTICK + 7: state_.l2_pressed = pressed; break;
    case BTN_JOYSTICK + 8: state_.select_pressed = pressed; break;
    case BTN_JOYSTICK + 9: state_.start_pressed = pressed; break;
    case BTN_JOYSTICK + 10: state_.r3_pressed = pressed; break;
    case BTN_JOYSTICK + 11: state_.l3_pressed = pressed; break;
    case BTN_GAMEPAD + 3: state_.plus_pressed = pressed; break;
    case BTN_GAMEPAD + 4: state_.minus_pressed = pressed; break;
    case BTN_GAMEPAD + 5: state_.encoder_clockwise_pressed = pressed; break;
    case BTN_GAMEPAD + 6:
      state_.encoder_counter_clockwise_pressed = pressed;
      break;
    case BTN_GAMEPAD + 7: state_.encoder_confirm_pressed = pressed; break;
    default: break;
  }
}

// 将方向盘原始轴线性缩放并钳制到 [-1.0, 1.0]，负值向左，正值向右
double InputEventProcessor::normalizeSteering(std::int32_t value,
                                              AxisRange range) {
...
double InputEventProcessor::normalizePedal(std::int32_t value,
                                           AxisRange range) {
  if (range.minimum >= range.maximum) return 0.0;
...
PovDirection InputEventProcessor::hatToDirection(std::int32_t x,
                                                  std::int32_t y) {
...
// 累积单个输入事件并在帧结束时返回完整状态
std::optional<InputDeviceState> InputEventProcessor::consume(
  ...
  if (event.type == EV_ABS) {
    ...
    setButton(event.code, event.value != 0);
  } else if (event.type == EV_SYN && event.code == SYN_REPORT) {
    // SYN_REPORT 表示本批事件结束，返回此时的完整设备状态
    return state_;
  }
  return std::nullopt;
}
...
// 关闭持有的输入设备
InputDeviceReader::~InputDeviceReader() { closeDevice(); }

// 将 errno 转为便于日志定位的错误文本
std::string InputDeviceReader::systemError(const std::string &action) {
  return action + ": " + std::strerror(errno);
}

// 关闭当前设备并标记为不可用
void InputDeviceReader::closeDevice() {
  if (fd_ >= 0) close(fd_);
  fd_ = -1;
}
...
// 读取当前所有待处理事件，每遇到 SYN_REPORT 输出一帧完整状态
std::vector<InputDeviceState> InputDeviceReader::readStates() {
  ...
  while (true) {
    ...
    for (std::size_t index = 0; index < count; ++index) {
      if (auto state = event_processor_.consume(events[index]))
