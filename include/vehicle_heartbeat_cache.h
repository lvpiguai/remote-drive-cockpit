...
class VehicleHeartbeatCache {
 public:
  ...
  explicit VehicleHeartbeatCache(Clock::duration online_timeout);
  ...
  // 校验车辆通信地址
  bool matchesVehicleAddress(const std::string &vehicle_id,
                             const sockaddr_in &vehicle_address) const;
 ...
 private:
  struct HeartbeatRecord {
    std::optional<sockaddr_in> vehicle_address;
